...
def Load_Data(dir_path):
    ...
    for idx in range(file_num):
        for idx_ in range(2):
            ...
            data_ori=T_PowerGain.tpowGain(data_ori,np.arange(1500)/4,2)
...
if __name__=='__main__':
    ...
    ImageShape=(256,256)
    epochs=600
    WaveUNet=Wave_U_Net_.Wave_U_Net(image_shape=(256,256,1))
    WaveUNet_model=WaveUNet.Build_Wave_U_Net()
    
    inputs_train=data_ori
    outputs_train=data_L
    inputs_validation=data_ori[:5]
    outputs_validation=data_L[:5]
